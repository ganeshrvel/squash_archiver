...
func packTarBrotli(arc *CommonArchive, newArchiveFile *archiver.TarBrotli, fileList []string, commonParentPath string) error {
	...
	for absolutePath, item := range zipFilePathListMap {
		...
		if err := addFileToTarBrotliArchive(newArchiveFile, item.fileInfo, item.absFilepath, item.relativeFilePath, item.isDir)
			err != nil {
			return err
		}
	}
...
func addFileToTarBrotliArchive(newArchiveFile *archiver.TarBrotli, fileInfo os.FileInfo, filename string, relativeFilename string, isDir bool) error {
